...
- `src/hooks/useDueTaskNotifications.ts` — reminds about tasks due today via the plain `Notification` constructor (not `ServiceWorkerRegistration.showNotification()`), deduped per task per day in `localStorage` (key `task-pulse-notified-<date>`). This only fires while the app is open in a tab — there's no push infrastructure — and the plain constructor doesn't work on Android Chrome at all (it requires a service worker there). A real background/mobile-capable version needs Firebase Cloud Messaging plus a scheduled Cloud Function, which requires the Blaze billing plan (still free at this app's scale, but needs a card on file) — that's a deliberately separate, not-yet-built piece.
- **Selection mode** — `App.tsx` owns `selectMode`/`selectedIds` (a plain `Set<string>`, not per-view) and threads them down through `SortableGroupItem`/`GroupItem`/`SortableTaskItem`/`TodayView`/`CompletedView` into `TaskItem`, which swaps its round done-toggle for a square checkbox and disables its own drag handle/delete/snooze buttons while `selectMode` is true. Selection persists across view switches (Groups/Today/Completed) since it's one global `Set`, and `App.tsx`'s bulk complete handler still checks each selected task's `recurrence` and calls `spawnNextOccurrence` itself — `bulkSetPercent` (`tasksApi.ts`) is a plain batched percent write with no recurrence awareness.
- **Snooze** (`TaskItem.tsx`'s clock-icon button, shown when a task has a due date and isn't done) always sets `dueDate` to tomorrow *relative to today*, not to the task's current due date — so a task overdue by a week snoozes to tomorrow, not to eight days from its stale due date.
